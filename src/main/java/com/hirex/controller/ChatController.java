...
public class ChatController {
    ...
    // ── Delete for me ────────────────────────────────────────────
    // Removes the message from the caller's own chat view only. The other
    // participant's view, and the underlying ChatMessage row, are untouched.

    @DeleteMapping("/messages/{messageId}/for-me")
    public ResponseEntity<ChatDto.DeleteForMeResponse> deleteForMe(
            @PathVariable Long messageId,
            Principal principal) {
        return ResponseEntity.ok(chatService.deleteForMe(messageId, principal.getName()));
    }

    // ── Delete for everyone ──────────────────────────────────────
    // Only the original sender, only within the configurable time window
    // (chat.delete-for-everyone.window-minutes). Broadcasts a WebSocket
    // event to every participant currently subscribed to this conversation.
