...
public interface ChatMessageRepository extends JpaRepository<ChatMessage, Long> {
    ...
    // NOT EXISTS against message_deletions excludes messages this user has
    // deleted-for-me directly in SQL — no row is fetched from the DB just to
    // be filtered out in Java afterwards (the old CSV-column approach had to
    // load every row, then call ChatMessage.isDeletedForUser() per row).
    @Query("SELECT m.id FROM ChatMessage m " +
            "WHERE m.application = :application " +
            "AND NOT EXISTS (" +
            "  SELECT 1 FROM MessageDeletion d " +
            "  WHERE d.message = m AND d.user = :viewer" +
            ") " +
            "ORDER BY m.sentAt ASC")
    Page<Long> findVisibleIdsByApplicationPaged(
    ...
    // Superseded for the main getMessages() flow by findVisibleIdsByApplicationPaged
    // (which also excludes delete-for-me rows in SQL), but kept for any
    // call site that needs the unfiltered full id list (e.g. admin/export tools).
